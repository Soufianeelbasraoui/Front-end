...
import './Lesoffers.css';

function Lesoffers({ handelAddtoCart}) {
  const [filter, setFilter] = useState('all');
  const [cart, setCart] = useState([]);

  // Filtrer les produits ayant une promo et selon la catégorie
  const filteredProducts = Featuredproducts.filter(product => 
    product.promo // Only include products with a promo field
  ).filter(product => 
  ...
  return (
    <div className="offers-container">
      ...
      <div className="filter-container">
        ...
        <button 
          className={`filter-btn ${filter === 'Électroniques' ? 'active' : ''}`}
          onClick={() => setFilter('Électroniques')}
        >
          Électroniques
        </button>
        <button 
          className={`filter-btn ${filter === 'Meubles' ? 'active' : ''}`}
          onClick={() => setFilter('Meubles')}
        >
          Meubles
        </button>
        <button 
          className={`filter-btn ${filter === 'Vêtements' ? 'active' : ''}`}
          onClick={() => setFilter('Vêtements')}
        >
          Vêtements
        </button>
        <button 
          className={`filter-btn ${filter === 'Sacs et Bagages' ? 'active' : ''}`}
          onClick={() => setFilter('Sacs et Bagages')}
        >
          Sacs & Bagages
        </button>
      </div>
